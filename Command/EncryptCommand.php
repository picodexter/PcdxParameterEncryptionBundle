...
{
    /**
     * @var EncryptDispatcherInterface
     */
    private $dispatcher;
     ...
     * Constructor.
     *
     * @param EncryptDispatcherInterface $dispatcher
     */
    public function __construct(EncryptDispatcherInterface $dispatcher)
    {
    ...
    {
        $this->setName('pcdx-parameter-encryption:encrypt')
            ->setDescription('Encrypt a value with a specified algorithm.')
            ->addArgument('algorithm_id', InputArgument::REQUIRED, 'Algorithm ID')
            ->addArgument(
                'plaintext_value',
                InputArgument::OPTIONAL,
                'Plaintext value (optional, will be prompted for if not supplied)'
            )
            ->addOption(
                'key',
                'k',
